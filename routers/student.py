...
@router.post("/create")
async def create_student(request:Request, student:CreateStudent):
    ...
    new_student = Students(firstname=student.firstname, lastname=student.lastname, matric_no=student.matric_no)

    new_student.save()

    return CustomResponse("created student successfully", status=status.HTTP_201_CREATED)
